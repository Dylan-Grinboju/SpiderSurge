...
public class PlayerControlSettings : MonoBehaviour
{
    ...
    private static readonly Dictionary<int, string> playerAbilityButtons = [];

    // Track menu button hold state per player
    private readonly Dictionary<int, float> menuButtonHoldStartTime = [];
    private readonly Dictionary<int, bool> menuButtonHeld = [];

    // Track button presses for double-tap detection
    private readonly Dictionary<int, string> lastButtonPressed = [];
    private readonly Dictionary<int, float> lastButtonPressTime = [];

    // Visual feedback tracking
    private readonly Dictionary<int, string> feedbackMessages = [];
    private readonly Dictionary<int, float> feedbackEndTimes = [];
    ...
    private static readonly string[] SupportedButtons =
    [
    ...
    private readonly Dictionary<string, InputAction> buttonActions = [];
    ...
    public static string GetPlayerAbilityButton(int playerIndex) => playerAbilityButtons.TryGetValue(playerIndex, out string button) ? button : DefaultGamepadButton;
    ...
    private void OnGUI()
    {
        ...
        foreach (var kvp in feedbackMessages)
        {
            ...
            if (feedbackEndTimes.TryGetValue(playerIndex, out float endTime) && Time.time < endTime)
            {
                ...
                float width = 400f;
                float height = 40f;
                Rect bgRect = new(Screen.width / 2 - width / 2, yOffset + (playerIndex * 50), width, height);
